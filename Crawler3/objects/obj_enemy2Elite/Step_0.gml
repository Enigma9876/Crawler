...
if(hp <= 0)
{
	...
	instance_create_layer(x, y, "Instances_Enemies", obj_key);
		death = true;
		sprite_index = spr_enemy2DeathElite;
		image_index = 0;
	}
...
else
{
	   ...
	   {   
		   ...
		   if((x - 16 - (room_width div 4)) div 32 == playerx || (y - 16 - (room_height div 4)) div 32 == playery)
		   {
			   ...
			   else if(obj_player.x > x)
			   {
				...
				var fireball = instance_create_layer(x, y, "Instances_projectiles", obj_fireball2);
				if((x - 16 - (room_width div 4)) div 32 == playerx && (y - 16 - (room_height div 4)) div 32 > playery)
				{
					fireball.image_angle = 90;
					fireball.vspeed = -10;
				}
				else if((x - 16 - (room_width div 4)) div 32 == playerx && (y - 16 - (room_height div 4)) div 32 < playery)
				{
					fireball.image_angle = 270;
					fireball.vspeed = 10;
				}
				else if((y - 16 - (room_height div 4)) div 32 == playery && (x - 16 - (room_width div 4)) div 32 > playerx)
				{
	...
	if(sprite_index == spr_enemy2AttackElite && image_index > 2)
	{
		attacking = false;
		sprite_index = spr_enemy2Elite;
	}
	
	if(sprite_index == spr_enemy2Elite && attacking)
	{
		sprite_index = spr_enemy2AttackElite;
		image_index = 0;
...
if(sprite_index == spr_enemy2DeathElite && image_index > 3 && death)
{
	instance_destroy(id, false);
}

if(sprite_index == spr_enemy2Elite || sprite_index == spr_enemy2Elite)
{
	isIdle = true;
}
else
